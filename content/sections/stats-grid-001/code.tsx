      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid grid-cols-1 gap-px overflow-hidden border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="group relative bg-background p-8 transition-all hover:bg-muted/50"
            >
              {/* コーナードット */}
              <div className="absolute left-2 top-2 h-1 w-1 rounded-full bg-border transition-colors group-hover:bg-muted-foreground" />
              <div className="absolute right-2 top-2 h-1 w-1 rounded-full bg-border transition-colors group-hover:bg-muted-foreground" />
              <div className="absolute bottom-2 left-2 h-1 w-1 rounded-full bg-border transition-colors group-hover:bg-muted-foreground" />
              <div className="absolute bottom-2 right-2 h-1 w-1 rounded-full bg-border transition-colors group-hover:bg-muted-foreground" />

              {/* 数値 */}
              <p className="text-4xl font-light tracking-wider text-foreground">
                {stat.value}
              </p>
